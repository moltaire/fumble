    ...
    if args.reassess:
        ...
        for i, a in enumerate(candidates, 1):
            ...
            try:
                ...
                old = f"[{a.suggestion}] ({a.domain_fit}, {a.role_fit})" if a.suggestion else "unrated"
                new = f"[{result.suggestion}] ({result.domain_fit}, {result.role_fit})"
                print(f"  {old} -> {new} — {result.job_summary}")
                ok += 1
